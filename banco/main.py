...
if os.path.exists('arquivos_banco.json'):
    re_instance_dados('arquivos_banco.json', contas_clientes)
...
while True:
    ...
    match menu('ABRIR CONTA', 'ACESSAR CONTA', 'ENCERRAR SESSÃO'):
        ...
        case 1:  # ABRIR CONTA
            ...
            try:
                nome_cliente = name_validation()
                cpf_cliente = cpf_validation(contas_clientes)
                senha_cliente = password_check('Senha de 4 dígitos: ')
                deposito_inicial = number_check('Valor de Depósito: R$')
